...
int save_user(USER temp)
{
	...
	if((fp=fopen("userinfo.dat","rb+"))==NULL)		//首次使用时应创建文件
	{
		...
		puthz(200, 200, "无法打开文件", 24, 24, DARKGRAY);
		delay(5000);
		exit(1);
	}
...
void Getinfo(int x1,int y1,char *name,int num,int a1,int b1,int c1,int d1)
{
	...
	while(1)     													//当按下回车键时表示输入完毕（回车键值为13）
    {
		...
		if(bioskey(1)==0||(mouse_press(a1,b1,c1,d1)==1))			//延时函数，使光标闪烁，在点击鼠标或有输入时退出
		{
			for(k=0;k<100;k++)				//延时的同时检测鼠标键盘
			{
				delay(2);
				if(bioskey(1)|| (mouse_press(a1, b1, c1, d1) == 1))
				{
					break;
				}
			}
		...
		if(bioskey(1))
		{
			...
			if(temp!='\r'&&temp!='\n')	//检测输入不为回车键，则继续，否则输入结束
			{
				...
				{
					bar1(border,y1,border+8, y1+16, WHITE);
		...
		if(mouse_press(a1,b1,c1,d1)==0)		//如果鼠标点击输入框外的部分，退出
		{
			break;
		}
		bar1(border,y1,border+8, y1+16, WHITE);
